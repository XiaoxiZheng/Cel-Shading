...
inline STPoint2 operator+(const STPoint2& left, const STPoint2& right)
{
    return STPoint2(left.x + right.x, left.y + right.y);
}

inline STPoint2 operator*(const STPoint2& left, const float& right)
{
    return STPoint2(left.x * right, left.y * right);
}
